...
-(void) initProductScrollView
{
    ...
    for (int i=0; i<[self.m_rRoomProObjArr count]; i++) {
        ...
        UIButton* button = [[UIButton alloc] initWithFrame:CGRectMake(startoff + 4*1, 0, 170, 140)];
        button.tag = i;
        [button addTarget:self action:@selector(imageSelectButton:) forControlEvents:UIControlEventTouchUpInside];
        ...
        if (i == 0) {
            ...
            [self.m_selectBulbImageView setCenter:CGPointMake(114, 150)];
    ...
    [self.m_imageScrollView setContentSize:CGSizeMake(startoff+4, 100)];
}
...
- (void)loadScrollViewWithPage:(int)page
{
    ...
    self.m_imageInfoLabel.text = imageobj.m_description;
