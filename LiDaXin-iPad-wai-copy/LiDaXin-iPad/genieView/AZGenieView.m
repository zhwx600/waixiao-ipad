...
- (void)animationDone1:(NSTimer*)timer
{
    [self.m_realView setHidden:NO];
}
...
- (void)genieAnimationShow:(bool)showing withDuration:(NSTimeInterval)duration
{
    ...
    [self insertSubview:animationView atIndex:201];
    [animationView release];
   // [self addSubview:animationView];
    
    if (showing)
        [NSTimer scheduledTimerWithTimeInterval:duration-0.1 target:self selector:@selector(animationDone1:) userInfo:nil repeats:NO];
